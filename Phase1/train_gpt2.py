...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None, return_all_hidden=False, return_block9=False):
        B, T = idx.size()
        ...
        for i, block in enumerate(self.transformer.h):
            ...
            if return_all_hidden:
                all_hidden.append(x)
            if return_block9 and i == 9:
                block9_out = x
        x = self.transformer.ln_f(x)
        logits = self.lm_head(x)
        loss = None
        if return_all_hidden and return_block9:
            return logits, loss, all_hidden, block9_out
